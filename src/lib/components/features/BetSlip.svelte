...
{#if sheetOpen}
	<div
		class="xl:hidden fixed inset-0 z-[95] animate-fade-in"
	...
	<div
		bind:this={sheetEl}
		class="xl:hidden fixed bottom-0 left-0 right-0 z-[100] rounded-t-2xl flex flex-col animate-slide-up"
		role="dialog"
		aria-modal="true"
		aria-label="Bet slip"
	...
	>
		...
		>
			<div class="flex items-center gap-2.5">
				<span class="font-display text-[17px] tracking-[2px]" style="color:var(--color-text);"
					>BET SLIP</span
				>
				<span
			...
			<div class="flex items-center gap-3">
				<button
					onclick={() => {
						if ($selectionCount > 0) betSlip.clear();
					}}
		...
		<!-- Tabs -->
		<div
			class="flex shrink-0 px-4 pt-3 gap-0"
		...
		<div class="overflow-y-auto flex-1 px-4 py-3 flex flex-col gap-2.5" style="min-height:100px;">
			{#each $selections as sel (sel.oddId)}
				<div
					class="relative rounded-xl p-3.5 animate-slide-in-right"
				...
				>
					<button
						...
						><X size={10} /></button
					>
					<div class="text-[10px] mb-1 pr-8 truncate" style="color:var(--color-text-muted);">
						{sel.matchLabel}
					</div>
					<div class="flex items-center justify-between pl-2">
						<span class="text-[13px] font-semibold" style="color:var(--color-text);"
							>{sel.pick}</span
						>
						<span class="font-mono text-[16px] font-bold" style="color:var(--color-gold);"
		...
		<!-- Footer -->
		<div
			class="px-4 pb-8 flex flex-col gap-3 shrink-0"
		...
		>
			...
			<div class="flex gap-1.5">
				{#each QUICK_STAKES as qs}
					<button
						...
						style="background:{$stake === qs
							? 'rgba(240,192,64,0.12)'
							: 'rgba(255,255,255,0.04)'}; border:1px solid {$stake === qs
							? 'rgba(240,192,64,0.3)'
							: 'var(--color-border)'}; color:{$stake === qs
							? 'var(--color-gold)'
							: 'var(--color-text-dim)'};"
					>
						{qs}
			...
			<div class="flex items-center gap-3">
				<span class="text-[12px] whitespace-nowrap" style="color:var(--color-text-dim);"
					>Stake (KSh)</span
				>
				<input
					type="number"
					...
					style="background:var(--color-card); border:1px solid {stakeInvalid && $stake > 0
						? 'rgba(239,68,68,0.5)'
						: 'var(--color-border)'}; color:var(--color-text);"
					onfocus={(e) =>
						((e.currentTarget as HTMLElement).style.borderColor = 'rgba(240,192,64,0.35)')}
			...
			>
				...
				>
					...
					<span class="font-mono" style="color:var(--color-green);"
						>{formatKES($potentialWin > 0 ? $potentialWin : $stake * $totalOdds)}</span
					>
				</div>
			</div>
