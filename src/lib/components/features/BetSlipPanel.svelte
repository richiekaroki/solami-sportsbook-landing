...
<div class="flex flex-col h-full overflow-hidden" style="background:var(--color-nav);">
		...
		<div class="flex shrink-0 px-3 pt-3 gap-0" style="border-bottom:1px solid rgba(255,255,255,0.04);">
			...
			{:else}
				{#each $selections as sel (sel.oddId)}
					<div class="relative rounded-xl p-3 animate-slide-in-right"
						...
						><X size={10} /></button>
						<div class="text-[10px] mb-0.5 pr-6 truncate" style="color:var(--color-text-muted);">{sel.matchLabel}</div>
						<div class="text-[10px] mb-1.5 truncate" style="color:var(--color-text-dim);">{sel.competition}</div>
						<div class="flex items-center justify-between pl-2">
							<span class="text-[12px] font-semibold truncate" style="color:var(--color-text);">{sel.pick}</span>
							<span class="font-mono text-[14px] font-bold ml-2" style="color:var(--color-gold);">{formatOdds(sel.odds)}</span>
						</div>
					</div>
				{/each}
			{/if}
		...
		<div class="px-3 pb-4 flex flex-col gap-2 shrink-0"
			...
			<div class="flex items-center gap-2">
				<span class="text-[11px] whitespace-nowrap" style="color:var(--color-text-dim);">Stake (KSh)</span>
			...
			{#if $taxBreakdown && $selectionCount > 0}
				...
				>
						<span class="text-[11px] font-semibold" style="color:var(--color-text-dim);">Payout breakdown (KRA)</span>
						{#if showTax}<ChevronUp size={13} style="color:var(--color-text-muted);"/>{:else}<ChevronDown size={13} style="color:var(--color-text-muted);"/>{/if}
					</button>

					{#if showTax}
						<div class="px-3 pb-3 flex flex-col gap-1" style="border-top:1px solid rgba(255,255,255,0.04);">
							...
							<div class="flex justify-between text-[10px]">
								...
								<span class="font-mono" style="color:var(--color-red);">-{formatKES($taxBreakdown.excise)}</span>
							</div>
							<div class="flex justify-between text-[10px]" style="border-top:1px solid rgba(255,255,255,0.03); padding-top:4px;">
								<span style="color:var(--color-text-muted);">Net stake</span>
								<span class="font-mono" style="color:var(--color-text-dim);">{formatKES($taxBreakdown.netStake)}</span>
							</div>
							<div class="flex justify-between text-[10px]">
								<span style="color:var(--color-text-muted);">Gross winnings</span>
								<span class="font-mono" style="color:var(--color-text-dim);">{formatKES($taxBreakdown.grossWin)}</span>
							</div>
							<div class="flex justify-between text-[10px]">
								<span style="color:var(--color-text-muted);">WHT (20%)</span>
								<span class="font-mono" style="color:var(--color-red);">-{formatKES($taxBreakdown.wht)}</span>
							</div>
							<div class="flex justify-between text-[11px] font-bold pt-1" style="border-top:1px solid var(--color-border);">
								<span style="color:var(--color-text);">Net payout</span>
								<span class="font-mono" style="color:var(--color-green);">{formatKES($taxBreakdown.netPayout)}</span>
							</div>
						</div>
					{/if}
				</div>
				...
				{#if !showTax}
					<div class="flex items-center justify-between px-1">
						<span class="text-[11px]" style="color:var(--color-text-dim);">Net payout (after tax)</span>
			...
			{:else}
				<div class="flex items-center justify-between px-1">
					...
					<span class="font-mono text-[14px] font-bold" style="color:var(--color-green);">KSh 0.00</span>
				</div>
			{/if}
			...
			<div class="rounded-xl p-3 flex flex-col gap-2"
				...
				{#if $taxBreakdown}
					<div class="flex items-center justify-between text-[11px]">
						...
						<span class="font-mono" style="color:var(--color-red);">-{formatKES($taxBreakdown.excise + $taxBreakdown.wht)}</span>
					</div>
				{/if}
