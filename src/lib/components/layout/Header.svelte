...
<header
	class="fixed top-0 left-0 right-0 z-50 h-[56px] glass"
...
>
	...
	<div class="hidden xl:flex items-center h-full w-full">
		<a
			href="/"
			class="shrink-0 flex items-center gap-2.5 px-5 h-full"
			style="width:240px; border-right:1px solid rgba(255,255,255,0.06);"
		>
			<div class="relative w-5 h-5 flex items-center justify-center">
				<span
					class="absolute w-3.5 h-3.5 rounded-full animate-pulse-dot opacity-40"
		...
		<div
			class="flex-1 flex items-center gap-1 px-4 h-full overflow-x-auto"
		...
		<div
			class="shrink-0 flex items-center justify-end gap-2 px-4 h-full"
		...
		>
			{#if $selectionCount > 0}
				...
				>
					<span
						class="w-5 h-5 rounded-full text-[11px] font-bold flex items-center justify-center"
			...
			{/if}
			<button
				onclick={() => handleJoin('header_login')}
			...
			>
			<button
				onclick={() => handleJoin('header_join')}
	...
	<div class="xl:hidden flex items-center h-full w-full px-3 gap-2 min-w-0">
		...
		<button
			class="shrink-0 min-w-[44px] min-h-[44px] p-2.5 rounded-lg cursor-pointer"
			aria-label={mobileSearchOpen ? 'Close search' : 'Search teams'}
			aria-expanded={mobileSearchOpen}
			style="background:{mobileSearchOpen
				? 'rgba(240,192,64,0.12)'
				: 'rgba(255,255,255,0.06)'}; color:{mobileSearchOpen
				? 'var(--color-gold)'
				: 'var(--color-text-dim)'};"
			onclick={() => {
		...
		{#if $selectionCount > 0}
			<span
				class="shrink-0 w-5 h-5 rounded-full text-[11px] font-bold flex items-center justify-center"
				style="background:var(--color-gold); color:var(--color-nav);"
			>
				{$selectionCount}
			</span>
...
{#if mobileMenuOpen}
	<div
		class="xl:hidden fixed top-[56px] left-0 right-0 z-40 px-4 py-3 flex flex-col gap-2 animate-slide-up glass"
		style="border-bottom:1px solid rgba(255,255,255,0.06);"
	>
		<button
			onclick={() => {
				handleJoin('mobile_login');
				mobileMenuOpen = false;
			}}
			aria-label="Log in to your account"
...
{#if mobileSearchOpen}
	<div
		class="xl:hidden fixed top-[56px] left-0 right-0 z-40 px-4 py-3 animate-slide-up glass"
		style="border-bottom:1px solid rgba(255,255,255,0.06);"
	>
		<div
