...
<div class="flex gap-1.5 overflow-x-auto pb-1" style="scrollbar-width:none;">
	{#each tabs as tab (tab.id)}
		...
		>
			<span
				class="w-4 h-4 rounded flex items-center justify-center text-[7px] font-bold"
			...
			<span
				...
				style="background:{activeComp === tab.id
					? 'rgba(240,192,64,0.15)'
					: 'rgba(255,255,255,0.05)'}; color:{activeComp === tab.id
					? 'var(--color-gold)'
