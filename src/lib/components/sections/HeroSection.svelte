<script lang="ts">
	...
	const trustChips = [
		{ icon: Zap,       label: 'M-PESA Instant',  sub: 'Deposits & withdrawals', accent: 'var(--color-green)' },
...
<div class="relative overflow-hidden rounded-2xl mb-5"
	...
	<div class="pointer-events-none absolute inset-0 overflow-hidden">
		<div class="absolute -top-20 -left-16 w-80 h-80 rounded-full blur-[80px]"
			style="background:radial-gradient(circle, rgba(240,192,64,0.10) 0%, transparent 65%);"></div>
		<div class="absolute -bottom-16 right-0 w-56 h-56 rounded-full blur-[60px]"
			style="background:radial-gradient(circle, rgba(34,197,94,0.07) 0%, transparent 65%);"></div>
	</div>

	<div class="relative flex flex-col sm:flex-row items-center gap-6 px-6 py-7">
		<div class="flex-1 min-w-0">
			<div class="flex items-center gap-2 mb-4">
				<span class="flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-[1.2px] px-2.5 py-1 rounded-full"
			...
			<h1 class="font-display leading-[0.9] tracking-[2px] mb-3"
				style="font-size:var(--text-display); color:white; text-wrap:balance;">
				BET ON EVERY<br />
				<span style="color:var(--color-gold);">MATCH DAY.</span>
			</h1>

			<p class="text-[13px] mb-5 leading-relaxed max-w-[340px]" style="color:var(--color-text-dim);">
			...
			<div class="flex items-center gap-3 flex-wrap">
				<button
					onclick={() => { trackCTA('hero_join_now', 'hero_section'); onJoin(); }}
		...
		<div class="shrink-0 w-full sm:w-auto sm:min-w-[230px] flex flex-col gap-2">
			...
			<div class="flex items-center gap-3 px-4 py-3 rounded-xl"
				...
				<div>
					<div class="text-[18px] font-bold leading-tight" style="color:var(--color-text);">{trustStats[0].value}</div>
					<div class="text-[10px]" style="color:var(--color-text-muted);">{trustStats[0].label}</div>
				</div>
			</div>
			<!-- Secondary chips row -->
			<div class="flex gap-2">
				{#each trustChips as chip}
					<div class="flex-1 flex items-center gap-2 px-3 py-2 rounded-lg"
						style="background:var(--color-card); border:1px solid var(--color-border);">
						<chip.icon size={13} style="color:{chip.accent}; flex-shrink:0;" />
						<div>
							<div class="text-[10px] font-bold leading-tight" style="color:var(--color-text);">{chip.label}</div>
							<div class="text-[8px] leading-tight" style="color:var(--color-text-muted);">{chip.sub}</div>
						</div>
					</div>
				{/each}
			</div>
