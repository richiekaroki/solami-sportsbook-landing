<script lang="ts">
	import { X, Eye, EyeOff, Phone, Lock, Smartphone, Gift, Crosshair, CheckCircle, Key } from 'lucide-svelte';
	...
	$effect(() => { phoneRaw = formatKEPhone(phoneRaw); });
	...
	function handleSubmit() {
		...
		setTimeout(() => { loading = false; step = 'success'; }, 1800);
	}

	function handleClose() {
		onClose();
		setTimeout(() => {
			step = 'form'; phoneRaw = ''; mpesa = ''; password = ''; loading = false;
		}, 300);
	}

	$effect(() => { if (open) trackAuthModalOpen('header'); });

	function handleKeydown(e: KeyboardEvent) {
		if (e.key === 'Escape') { handleClose(); return; }
		if (e.key !== 'Tab' || !modalEl) return;
		const focusable = modalEl.querySelectorAll<HTMLElement>('button, input, [tabindex]:not([tabindex="-1"])');
	...
	>
		...
		>
			<div class="h-[3px] w-full"
				style="background:linear-gradient(90deg, var(--color-gold), var(--color-gold), var(--color-gold));"></div>
			...
			{#if step === 'form'}
				<div class="flex items-center justify-between px-6 pt-5 pb-4">
					<div>
						<div class="flex items-center gap-2 mb-0.5">
							...
							<span class="font-display text-[20px] tracking-[2px]" style="color:var(--color-text);">WAM</span>
						</div>
				...
				<div class="flex mx-6 mb-5 rounded-xl overflow-hidden"
				...
				{#if activeMode === 'signup'}
					<div class="mx-6 mb-4 flex items-center gap-2.5 px-3.5 py-2.5 rounded-xl"
						...
						<div>
							<div class="text-[12px] font-bold" style="color:var(--color-green);">Claim KSh 500 Welcome Bonus</div>
							<div class="text-[10px]" style="color:rgba(34,197,94,0.55);">Deposit KSh 100 via M-PESA to unlock</div>
						</div>
					</div>
				{/if}
				...
				<div class="px-6 flex flex-col gap-3">
					<div>
						...
						<div class="flex items-center gap-2 rounded-xl px-3.5 py-3 transition-all duration-150 focus-ring-gold"
							style="background:var(--color-card); border:1px solid var(--color-border);"
						>
							<Phone size={15} style="color:var(--color-text-muted); flex-shrink:0;" />
							<span class="text-[12px] font-mono shrink-0" style="color:var(--color-text-muted);">+254</span>
							<input
								id="auth-phone"
								type="tel"
								bind:value={phoneRaw}
						...
						<div class="text-[10px] mt-1 px-1" style="color:var(--color-text-muted);">Must start with 07 or 01</div>
					</div>

					{#if activeMode === 'signup'}
						<div>
							<label for="auth-mpesa"
								class="block text-[11px] font-semibold mb-1.5 uppercase tracking-[0.8px]"
								style="color:var(--color-text-muted);">
								M-PESA Number
							...
							<div class="flex items-center gap-2 rounded-xl px-3.5 py-3 transition-all duration-150 focus-ring-green"
								style="background:var(--color-card); border:1px solid var(--color-border);"
							>
								<Smartphone size={15} style="color:var(--color-green); flex-shrink:0;" />
								<span class="text-[11px] font-bold shrink-0" style="color:var(--color-green);">M-PESA</span>
								<input
									id="auth-mpesa"
									type="tel"
									bind:value={mpesa}
					...
					<div>
						...
						>
							...
							<input
								...
								style="color:var(--color-text);"
							/>
							<button onclick={() => (showPass = !showPass)} aria-label={showPass ? 'Hide password' : 'Show password'} aria-expanded={showPass} class="hover:text-[var(--color-text)] transition-colors cursor-pointer" style="color:var(--color-text-muted);" type="button">
					...
					>
						{#if loading}
							<span class="w-4 h-4 rounded-full border-2 border-t-transparent animate-spin"
								style="border-color:var(--color-nav); border-top-color:transparent;"></span>
							Verifying...
					...
					<p class="text-center text-[10px] pb-5 leading-relaxed" style="color:var(--color-text-muted);">
						{#if activeMode === 'signup'}
							By signing up you agree to our <span style="color:var(--color-text-dim);">Terms</span> &amp;
							<span style="color:var(--color-text-dim);">Privacy Policy</span>. 18+ only. Gamble responsibly.
							Licensed by <span style="color:var(--color-text-dim);">BCLB Kenya</span>.
						{:else}
			...
			{:else}
				<div class="px-6 py-10 flex flex-col items-center text-center gap-4">
					<div class="w-16 h-16 rounded-full flex items-center justify-center animate-slide-up"
					...
					<div class="animate-slide-up" style="animation-delay:0.08s;">
						<div class="font-display tracking-[2px] mb-1" style="font-size:var(--text-title); color:var(--color-text);">YOU'RE IN!</div>
						<div class="text-[13px]" style="color:var(--color-text-dim);">Account created successfully.</div>
					</div>
					<div class="w-full rounded-xl p-4 animate-slide-up"
						style="animation-delay:0.16s; background:rgba(34,197,94,0.06); border:1px solid rgba(34,197,94,0.15);">
						<div class="text-[12px] font-bold mb-1 flex items-center gap-1.5" style="color:var(--color-green);">
							<CheckCircle size={14} />
