...
<button
	...
	style="
		...
		box-shadow:{selected
		? '0 0 12px rgba(240,192,64,0.15), inset 0 1px 0 rgba(240,192,64,0.15)'
		: 'none'};
	"
>
	{#if !selected}
		<span
			class="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-150 rounded-lg"
			style="background:linear-gradient(135deg, rgba(240,192,64,0.06) 0%, transparent 60%);"
		></span>
	{/if}
	...
	<span
		class="relative font-bold leading-none mb-[3px] uppercase tracking-[0.5px]"
		style="font-size:{compactLabel ? '8px' : '9px'}; color:{selected
			? 'var(--color-gold)'
	...
	<span
		class="relative font-mono font-bold leading-none"
		style="font-size:13px; color:{valueColor}; text-shadow:{selected
	...
	{#if direction === 'up'}
		<span
			class="absolute top-[3px] right-[3px] text-[7px] leading-none"
			style="color:var(--color-green);">▲</span
		>
	{:else if direction === 'down'}
		<span
			class="absolute top-[3px] right-[3px] text-[7px] leading-none"
			style="color:var(--color-red);">▼</span
		>
	{/if}
</button>
