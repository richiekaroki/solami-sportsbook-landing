<script lang="ts">
	...
	const filtered = $derived((() => {
		...
		let list = activeComp === 'all' ? games : games.filter((g) => String(g.competition_id) === activeComp);
		if (query) {
			list = list.filter((g) =>
...
<svelte:head>
	...
	<meta name="description" content="Sports betting with instant M-PESA payouts. Bet on LaLiga, UCL, Premier League with 100+ markets per match. Licensed by BCLB Kenya." />
</svelte:head>
	...
	<main id="main-content" class="flex-1 min-w-0 overflow-x-hidden">
		...
		<div class="mb-4" id="featured">
			<div class="flex items-center gap-3 mb-2.5">
				<span class="text-[10px] font-bold uppercase tracking-[1.5px]" style="color:var(--color-text-muted);">Featured Match</span>
				<div class="flex-1 h-px" style="background:var(--color-border);"></div>
				<span class="flex items-center gap-1 text-[10px] font-semibold" style="color:var(--color-green);">
					<span class="w-1.5 h-1.5 rounded-full animate-pulse-dot" style="background:var(--color-green);"></span>
					Odds updating live
				</span>
			</div>
			...
			<div class="flex items-center gap-2.5 px-3.5 py-2.5 rounded-xl mb-2"
				...
				<span class="ml-auto text-[11px] font-semibold" style="color:var(--color-text-muted);">{filtered.length} events</span>
			</div>
			...
			<div style="min-width:0;">
				{#if !loaded}
					...
					<div class="flex items-center gap-2 mb-3 px-1">
						<div class="w-1.5 h-1.5 rounded-full animate-pulse-dot" style="background:var(--color-gold);"></div>
						<span class="text-[11px] font-semibold" style="color:var(--color-text-muted);">Loading matches...</span>
					</div>
					{#each [1,2,3] as _}
						<div class="mb-3">
							<div class="h-8 rounded-t-xl animate-pulse" style="background:rgba(255,255,255,0.03);"></div>
							<div class="rounded-b-xl overflow-hidden" style="background:var(--color-nav); border:1px solid var(--color-border); border-top:none;">
								{#each [1,2,3] as _}
									<div class="flex items-center gap-3 px-3 py-3" style="border-bottom:1px solid rgba(255,255,255,0.03);">
										<div class="flex-1 flex flex-col gap-1.5">
											<div class="h-3 rounded animate-pulse" style="background:rgba(255,255,255,0.04); width:60%;"></div>
											<div class="h-3 rounded animate-pulse" style="background:rgba(255,255,255,0.04); width:50%;"></div>
										</div>
										<div class="flex gap-1">
											{#each [1,2,3,4,5,6,7] as _}
												<div class="w-[44px] h-[44px] rounded-lg animate-pulse" style="background:rgba(255,255,255,0.03);"></div>
											{/each}
										</div>
									</div>
								{/each}
				...
				{:else if filtered.length === 0}
						<div class="flex flex-col items-center justify-center gap-3 py-16 text-center">
							...
							<div class="text-[14px] font-semibold" style="color:var(--color-text-muted);">No matches found</div>
							{#if $searchQuery}
						...
						{#each grouped as group (group.compId)}
							<div class="flex items-center justify-between px-3.5 py-2 mt-3 first:mt-0 rounded-t-xl"
								...
								<span class="text-[11px] font-bold uppercase tracking-[1.2px] flex items-center gap-1.5 min-w-0 flex-1"
									style="color:{group.compId===7 ? 'var(--color-blue)' : 'var(--color-red)'};">
									<span class="w-4 h-4 rounded flex items-center justify-center text-[7px] font-bold shrink-0"
										style="background:{group.compId===7 ? 'rgba(96,165,250,0.12)' : 'rgba(248,113,113,0.12)'};">
						...
						<div class="shrink-0 text-[10px] font-bold uppercase tracking-[0.8px]" style="width:168px; color:var(--color-text-muted);">Match</div>
						<div class="flex items-center gap-1 shrink-0">
							<span class="text-[10px] font-bold uppercase tracking-[0.5px] mr-0.5 border-b border-dotted cursor-help" style="color:var(--color-text-muted); border-color:var(--color-text-muted);">1X2</span>
							<Tooltip text="Match result: Home win (1), Draw (X), or Away win (2)" />
							{#each ['1','X','2'] as h}<div class="text-[10px] font-bold text-center" style="width:44px; color:var(--color-text-muted);">{h}</div>{/each}
						</div>
						<div class="mx-1.5 shrink-0 w-px h-4" style="background:rgba(255,255,255,0.04);"></div>
						<div class="flex items-center gap-1 shrink-0">
							<span class="text-[10px] font-bold uppercase tracking-[0.5px] mr-0.5 border-b border-dotted cursor-help" style="color:var(--color-text-muted); border-color:var(--color-text-muted);">DC</span>
							<Tooltip text="Double Chance: Cover two outcomes — Home or Draw (1X), Home or Away (12), Draw or Away (X2)" />
							{#each ['1orX','1or2','Xor2'] as h}<div class="text-[9px] font-bold text-center" style="width:44px; color:var(--color-text-muted);">{h}</div>{/each}
						</div>
						<div class="mx-1.5 shrink-0 w-px h-4" style="background:rgba(255,255,255,0.04);"></div>
						<div class="flex items-center gap-1 shrink-0">
							<span class="text-[10px] font-bold uppercase tracking-[0.5px] mr-0.5 border-b border-dotted cursor-help" style="color:var(--color-text-muted); border-color:var(--color-text-muted);">GG/NG</span>
							<Tooltip text="Both Teams to Score: Yes (both score) or No (at least one doesn't)" />
							{#each ['Yes','No'] as h}<div class="text-[10px] font-bold text-center" style="width:44px; color:var(--color-text-muted);">{h}</div>{/each}
						</div>
		...
		<!-- 5. HOW TO BET -->
		<div class="mt-6 rounded-xl px-5 py-4"
			...
			<div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
				<div class="flex items-start gap-2.5">
					...
					<div>
						<div class="text-[12px] font-semibold" style="color:var(--color-text);">Pick your odds</div>
						<div class="text-[11px]" style="color:var(--color-text-muted);">Tap any odds button to add it to your slip</div>
					</div>
				</div>
				<div class="flex items-start gap-2.5">
					<span class="w-6 h-6 rounded-full flex items-center justify-center text-[11px] font-bold shrink-0 mt-0.5"
					...
					<div>
						<div class="text-[12px] font-semibold" style="color:var(--color-text);">Set your stake</div>
						<div class="text-[11px]" style="color:var(--color-text-muted);">Enter amount or use quick-stake buttons</div>
					</div>
				</div>
				<div class="flex items-start gap-2.5">
					<span class="w-6 h-6 rounded-full flex items-center justify-center text-[11px] font-bold shrink-0 mt-0.5"
					...
					<div>
						<div class="text-[12px] font-semibold" style="color:var(--color-text);">Place & win</div>
						<div class="text-[11px]" style="color:var(--color-text-muted);">Confirm your bet, payouts via M-PESA in minutes</div>
					</div>
				</div>
			</div>
			<div class="mt-3 pt-3 flex flex-col gap-2 text-[10px]" style="border-top:1px solid var(--color-border); color:var(--color-text-muted);">
				<div class="flex items-center gap-4">
					<span>Keyboard: <kbd class="px-1 py-0.5 rounded text-[9px] font-mono" style="background:rgba(255,255,255,0.06);">←→</kbd> navigate <kbd class="px-1 py-0.5 rounded text-[9px] font-mono" style="background:rgba(255,255,255,0.06);">Enter</kbd> select <kbd class="px-1 py-0.5 rounded text-[9px] font-mono" style="background:rgba(255,255,255,0.06);">↑↓</kbd> rows <kbd class="px-1 py-0.5 rounded text-[9px] font-mono" style="background:rgba(255,255,255,0.06);">1-4</kbd> quick stake</span>
					<span>Odds: <span style="color:var(--color-green);">▲</span> rising <span style="color:var(--color-red);">▼</span> falling</span>
				</div>
			</div>
		</div>
		...
		<footer class="mt-8 pt-8" style="border-top:1px solid var(--color-border);">
			...
			<div class="xl:hidden flex gap-3 mb-8 p-4 rounded-2xl"
				...
				<div class="flex-1">
					<div class="text-[14px] font-bold mb-0.5" style="color:var(--color-text);">Ready to bet?</div>
					<div class="text-[11px]" style="color:var(--color-text-muted);">Join free · Claim KSh 500 bonus</div>
				</div>
				<button
			...
			<div class="grid grid-cols-2 sm:grid-cols-4 gap-8 mb-8">
				<div>
					<div class="flex items-center gap-2 mb-3">
						...
						<span class="font-display text-[18px] tracking-[2px]" style="color:var(--color-text);">WAM</span>
					</div>
					<p class="text-[12px] leading-relaxed" style="color:var(--color-text-muted);">
						Sports betting with instant M-PESA payouts. Competitive odds across LaLiga, Champions League, and Premier League.
					</p>
				</div>
			{#each [
				...
				<div>
					<h4 class="text-[10px] font-bold uppercase tracking-[1.2px] mb-3" style="color:var(--color-text-muted);">{col.h}</h4>
			{#each col.links as l}
					<a href="#" class="block text-[12px] mb-2 transition-colors hover:text-white"
			...
			<div class="flex flex-wrap items-center justify-between gap-3 pt-4"
				...
				<p class="text-[11px]" style="color:var(--color-text-muted);">© 2026 WAM Ltd. Licensed by BCLB Kenya. 18+ only. Please gamble responsibly.</p>
				<div class="flex gap-2">
					{#each ['18+','BCLB','M-PESA','KE'] as b}
						<span class="text-[9px] font-bold uppercase px-2 py-1 rounded"
							style="background:rgba(255,255,255,0.03); color:var(--color-text-muted); border:1px solid var(--color-border);">{b}</span>
					{/each}
				</div>
			</div>
		</footer>
