...
test.describe('Responsive Layout – Desktop (1440px)', () => {
  ...
  test('desktop header with "Join Now" button is visible', async ({ page }) => {
    await expect(page.getByRole('button', { name: 'Join now and claim KSh 500 bonus' }).first()).toBeVisible();
  });

  test('header "Log In" button is visible', async ({ page }) => {
    await expect(page.getByRole('button', { name: 'Log in to your account' }).first()).toBeVisible();
  });
